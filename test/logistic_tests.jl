...
for i = 1:length(t)
    for Γ = 0:(i-1)
        ...
        Wp = rand(rng, 0: n)
        @assert(Γ < tp)
        objc, βc, zc = AdaptiveSurveillance.solve_logistic_Γ_subproblem_convex(Γ, vcat(t[1:i], tp), vcat(W[1:i], Wp), n, βu, logistic(p0u))
        ...
        obj, β, z = AdaptiveSurveillance.solve_logistic_Γ_subproblem(Γ, tp, Wp, t[1:i], W[1:i], n, βu, logistic(p0u))
...
plot_profile_likelihood(tp, t[1:ti], W[1:ti], n, βu, logistic(p0u), path = save_path)
