...
const p0_true = 0.01
const Γ_true = 100.0
const α_true = β_true * Γ_true + log(1 / p0_true - 1) # for standard logistic regression
...
const positive_counts = [1.0, 1.0, 1.0, 2.0, 4.0, 4.0, 3.0, 2.0, 2.0, 2.0, 0.0, 4.0, 3.0, 2.0, 2.0, 
...
n_samples = length(positive_counts)
...
# plot(p_log_true, xlabel = "Time", ylabel = "Prevalence", legend = false)
# savefig(joinpath(output_path, "change_example.pdf"))
# plot(positive_counts ./ n, xlabel = "Time", ylabel = "Positive Test Fraction", legend = false)
# savefig(joinpath(output_path, "change_signal.pdf"))
...
# @model logistic_epidemic_simple(n, positive_counts, test_times) = begin
#     β ~ Normal(0, 1)
#     α ~ Normal(0, 1)
#     for (i, t) in enumerate(test_times)
#         positive_counts[i] ~ BinomialLogit(n, β * t - α) # will put negative in front of all these things
#     end
# end

# # Inference
# # chain = sample(logistic_epidemic(n, positive_counts, test_times), NUTS(1000, 0.95), 2000)
...
# function llm(β, p0, Γ) # beta >= 0, p0 is a probability, gamma >= 0
#     l = 0.0
#     for t = 1:length(positive_counts)
#         if (t - 1) < Γ
#             l += positive_counts[t] * log(p0) + (n - positive_counts[t]) * log(1 - p0)
#         else
#             l += positive_counts[t] * (β * (t-1) - β * Γ + logit(p0)) - n * logistic(β * (t-1) - β * Γ + logit(p0))
#         end
#     end
#     return l
# end

# llm_100(β, p0) = llm(β, p0, Γ_true)
# b_grid = 0.001:0.01:1
# p0_grid = 0.001:0.01:1
# llm_grid = zeros(length(b_grid), length(p0_grid))
# for(i, b)  in enumerate(b_grid)
#     for (i, p) in enumerate(p0_grid)
#         llm_grid[i] = llm_100(b, p)
#     end
# end

# plot(b_grid, p0_grid, llm_100, st=:surface)

# function epidemic_loglikeli(β, p0, Γ, n, W, t) 
#     # @assert length(W) == length(t)
#     pe = logistic.(β .* t .- β * Γ .+ logit(p0))
#     return sum(W .* log.(max.(p0, pe)) .+ (n .- W) .* log.(1 .- max.(p0, pe)))
# end
...
@NLobjective(model, Max, sum(W[i] * log(pe[i]) + (n - W[i]) * log(1 - pe[i]) for i = 1:n_samples))
optimize!(model)

# next attempt: use optim package